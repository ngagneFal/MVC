...
This annotations allows you to specify a user-provided class to generate identifiers. This annotation only works when both :ref:`  <annref_id>` and :ref:`@GeneratedValue(strategy="CUSTOM") <annref_generatedvalue>` are specified.
...
instance variable which is annotated by :ref:`  <annref_id>`. This
annotation is optional and only has meaning when used in
conjunction with  .

If this annotation is not specified with   the NONE strategy is
used as default.
...
:ref:`  <annref_id>` is not supported.
